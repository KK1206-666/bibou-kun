...
export default function AppIntro() {
  const flow = [
    { num: '①', title: '備忘を登録', desc: '仕事・プライベートを選び、タイトルと期限（任意・時刻も設定可）を入力するだけ' },
    { num: '②', title: '分類・整理', desc: 'カテゴリ（プライベートのみ7種類）や「定常備忘」で繰り返しタスクも管理できる' },
    { num: '③', title: '通知を設定', desc: '定常備忘は曜日・時刻のリマインダー、備忘は期限に応じて自動でPush通知' },
  ...
  const features = [
    ...
    { icon: '🔁', title: '定常備忘（繰り返しタスク）', desc: '毎週・隔週、「毎月最終金曜日」のような月内の曜日、「毎月15日」のような日付指定にも対応' },
    { icon: '🔔', title: 'リマインダー通知', desc: '定常備忘に設定した曜日・時刻になると、ブラウザへPush通知が届く' },
    { icon: '⏰', title: '期限通知', desc: '備忘は期限の前日・当日に加え、超過後も完了にするまで毎日通知。時刻を設定すればその1時間前・10分前にも通知' },
  ...
  const notes = [
    ...
    'リマインダー通知は「定常備忘」のみ設定できます（備忘は期限通知でカバーします）',
    '期限通知は「備忘」のみが対象です（定常備忘には期限を設定できません）。期限に時刻を設定すると、その1時間前・10分前にも通知が届きます',
